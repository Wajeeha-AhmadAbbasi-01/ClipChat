...
if st.session_state.answer:
    ...
    # 1. Clean up any accidental HTML strings generated by the LLM
    clean_answer = st.session_state.answer
    clean_answer = clean_answer.replace('<div class="answer-text">', '')
    clean_answer = clean_answer.replace('</div>', '')
    clean_answer = clean_answer.strip()
    
    # 2. Render safely using clear, structured layout containers
    st.markdown("### 📝 Answer Summary")
    
    with st.container():
        st.info(f"**Q:** {question}")
        st.success(f"**A:** {clean_answer}")
